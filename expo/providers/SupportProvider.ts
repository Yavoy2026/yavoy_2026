...
const SYSTEM_PROMPT = `Ты дружелюбный AI-консультант мобильного приложения YAVAY — агрегатора туристических экскурсий по Узбекистану. Помогай подобрать тур, узнавай предпочтения (город, бюджет, интересы, длительность, сезон), рекомендуй экскурсии, объясняй условия бронирования. Отвечай кратко и по-русски. Города и направления YAVAY: Ташкент, Самарканд, Бухара, Хива, Чимган и Чарвак, Айдаркуль, Термез, Ферганская долина, Нукус, Муйнак и Арал. Цены указывай в сумах. Категории туров YAVAY: городские, познавательные, природные, паломничество, агротуры, фототуры, этнотуры, для родителей, глэмпинг, с животными, мистические, к диким животным, винные, гастро. Если пользователь просит говорить с менеджером, оператором или жалуется на ошибку/проблему, которую ты не можешь решить — ответь коротко: "ESCALATE: <причина>" в самом начале сообщения, затем извинись и сообщи, что переключаешь на менеджера.`;
  ...
  const [messages, setMessages] = useState<SupportMessage[]>([
    {
      id: "sup-welcome",
      role: "assistant",
      content: "Здравствуйте! Я AI-консультант YAVAY. Помогу подобрать экскурсию — расскажите, куда хотите поехать и какие интересы у вас?",
      ...
      setTimeout(() => {
        setMessages((prev) => [
          ...prev,
          {
            id: `agent-${Date.now()}`,
            role: "agent",
            content: "Менеджер YAVAY ответит вам в ближайшее время. Спасибо за ожидание!",
            createdAt: new Date().toISOString(),
          },
        ]);
      }, 600);
      ...
      if (shouldEscalate) {
        ...
        setMessages((prev) => [
          ...prev,
          {
            id: `sys-${Date.now()}`,
            role: "agent",
            content: "Чат переведён на администратора/менеджера YAVAY. Мы свяжемся с вами в течение нескольких минут.",
            createdAt: new Date().toISOString(),
          },
        ]);
      }
