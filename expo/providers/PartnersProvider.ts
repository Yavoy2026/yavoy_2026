...
const initialGuests: PartnerGuest[] = [
  { id: "pg1", tourId: "psub1", firstName: "Анна", lastName: "Соколова", phone: "+998 (91) 222-08-91", tourDate: "2026-04-20", ticketCount: 2, status: "completed" },
  { id: "pg2", tourId: "psub1", firstName: "Дмитрий", lastName: "Кузнецов", phone: "+998 (93) 401-77-12", tourDate: "2026-05-04", ticketCount: 3, status: "completed" },
  { id: "pg3", tourId: "psub1", firstName: "Мария", lastName: "Орлова", phone: "+998 (94) 678-34-09", tourDate: "2026-06-12", ticketCount: 4, status: "upcoming" },
  { id: "pg4", tourId: "psub2", firstName: "Сергей", lastName: "Васильев", phone: "+998 (97) 999-12-77", tourDate: "2026-06-22", ticketCount: 2, status: "upcoming" },
];

const initialTransactions: PartnerTransaction[] = [
  { id: "ptr1", tourId: "psub1", tourTitle: "Гастротур по базарам и чайханам Бухары", amount: 1700000, currency: "сум", date: "2026-04-20", guestName: "Анна Соколова", status: "completed" },
  { id: "ptr2", tourId: "psub1", tourTitle: "Гастротур по базарам и чайханам Бухары", amount: 2550000, currency: "сум", date: "2026-05-04", guestName: "Дмитрий Кузнецов", status: "completed" },
  { id: "ptr3", tourId: "psub1", tourTitle: "Гастротур по базарам и чайханам Бухары", amount: 3400000, currency: "сум", date: "2026-05-20", guestName: "Мария Орлова", status: "pending" },
  { id: "ptr4", tourId: "psub2", tourTitle: "Восхождение на Большой Чимган", amount: 840000, currency: "сум", date: "2026-05-22", guestName: "Сергей Васильев", status: "pending" },
  { id: "ptr5", tourId: "psub1", tourTitle: "Гастротур по базарам и чайханам Бухары", amount: 850000, currency: "сум", date: "2026-03-12", guestName: "Игорь Лебедев", status: "completed" },
  { id: "ptr6", tourId: "psub1", tourTitle: "Гастротур по базарам и чайханам Бухары", amount: 1700000, currency: "сум", date: "2025-12-10", guestName: "Наталья Котова", status: "completed" },
];
...
const initialChat: PartnerChatMessage[] = [
  ...
  { id: "pcm2", tourId: "psub1", authorType: "partner", authorName: "Партнёр", content: "Добрый день! Сбор в 9:00 у бассейна Ляби-Хауз, у Надира Диван-беги.", createdAt: "2026-04-18 14:32" },
  { id: "pcm3", tourId: "psub1", authorType: "admin", authorName: "Администратор YAVAY", content: "Подключился к диалогу для контроля качества. Всё на связи.", createdAt: "2026-04-18 14:33" },
];
...
export const [PartnersProvider, usePartners] = createContextHook(() => {
  ...
  const verifyAndRegister = useCallback(async (value: string): Promise<{ ok: boolean; error?: string }> => {
    ...
    try {
      ...
      if (!res.ok || !res.data) {
        return { ok: false, error: res.error ?? "Не удалось получить данные из налоговых органов" };
      }
      ...
      return { ok: false, error: "Ошибка соединения с налоговыми органами. Попробуйте позже." };
    } finally {
      setVerifying(false);
    }
  }, []);
    ...
    setEmailNotifications((list) => {
      ...
      return [
        ...list,
        {
          ...
          partnerInn: inn,
          email: target.email,
          subject: "Ваш партнёрский аккаунт YAVAY подтверждён",
          body: `Здравствуйте! Ваш аккаунт партнёра (ИНН ${inn}) успешно подтверждён администратором YAVAY. Теперь вы можете публиковать экскурсии и принимать бронирования.`,
    ...
    setEmailNotifications((list) => {
      ...
      return [
        ...list,
        {
          ...
          partnerInn: inn,
          email: target.email,
          subject: "Заявка партнёра YAVAY отклонена",
  ...
  const updateLegalDoc = useCallback((key: LegalDocKey, patch: { title?: string; body?: string }, notify: boolean) => {
    ...
    if (notify) {
      ...
      const mails: PartnerEmailNotification[] = recipients.map((r, i) => ({
        ...
        body: `Здравствуйте! Документ «${title}» был обновлён администратором YAVAY ${today}. Пожалуйста, ознакомьтесь с новой версией в кабинете партнёра.`,
        sentAt,
      }));
